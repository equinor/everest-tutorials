...
The control vector for `wellrate_experiment.yml` contains water injection rates for the wells **[A5, A6]** over three time intervals. Moreover, a hundred different realizations have been used to capture the uncertainty in the reservoir model.
...
.. important::
	...
	These time indexes have to be specified in the :file:`input/constraints_config.yml` file, in the format of one entry per time index per well.

.. literalinclude:: ../../../data/drogon/well_rate/everest/input/constraints_config.yml
  :language: yaml
...
The next job in this sequence of forward models is the **schmerge** job which processes the information of the previous two jobs (for example target rate and date) and writes an updated schedule file with all relevant information ready for simulation.

The final job is responsible for calculating NPV. The NPV input file ``well_rate/everest/input/prices.yml`` contains economic input parameters such as unit prices for oil production, water production and injection, drilling costs, discount factor, etc. 

.. important::

	For more detailed information and examples regarding the forward models please refer to the detailed `forward model documentation <https://everest.readthedocs.io/en/latest/forward_models/index.html>`_.
...
Experiment setup
*****************
In this experiment, we will alter the flow rates for different injection wells during a period of 7 years (from 2023 to 2030) to maximize an economic objective function, i.e., NPV. The control rates are defined three times in the 7-year simulated period every 730 days. Therefore, with two wells (we only vary the rates of the two injectors while keeping the controls of the producers fixed) and three control time intervals, we have a total of 2x3=6 control variables to be optimized. In this experiment uncertainty is taken into account by  using an ensemble of a 100 equally-plausible reservoir models and maximizing average NPV.
...
.. _figure_objectives_wr:
.. figure:: images/optimization/wr_objectives.svg
    :align: center
    :width: 90%

    Figure: Objective function over the iterations

The :ref:`figure_objectives_wr` shows average objective function at the
iterations of the optimization experiment (average NPV over all geological realizations).
The increase in objective function value of $2.95e+08 was achieved by changing
injection well rate targets over time from the initial guess.
...
.. figure:: images/production/wr_FOPT.svg
    :align: center
    :width: 90%
    :figclass: align-center
...
.. figure:: images/production/wr_FWIT.svg
    :align: center
    :width: 90%
    :figclass: align-center

    Figure: Cumulative water injection over time

.. figure:: images/production/wr_FPR.svg
    :align: center
    :width: 90%
    :figclass: align-center

    Figure: Field pressure development over time
...
.. figure:: images/production/wr_WWIR_A5.svg
    :align: center
    :width: 90%
    :figclass: align-center

    Figure: Water injection rates for well A5

.. figure:: images/production/wr_WWIR_A6.svg
    :align: center
    :width: 90%
    :figclass: align-center

    Figure: Water injection rates for well A6

The figure below compares the initial and optimal values for the controls. The figure shows the need for more injection in the first period of production. In later stages once the oil production rate drops the injection rate can be reduced. A more refined strategy can be obtained by having more time intervals, i.e., introducing additional control variables within the production time frame.

.. figure:: images/optimization/wr_controls.svg
    :align: center
    :width: 90%
    :figclass: align-center
